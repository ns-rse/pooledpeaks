test_that("check_fsa_v_batch correctly identifies number of .fsa files", {
  ...
  expect_true(any(grepl("12", output))) # Assuming there are 12 .fsa files in the test directory
})
...
test_that("check_fsa_v_batch correctly identifies version of .fsa files", {
  ...
  expect_true(any(grepl("-- Number of .fsa file formats present in batch:", output)))
  expect_true(any(grepl("v1.01, v3", output))) # Adjust the versions based on your test files
})
...
test_that("check_fsa_v_batch correctly identifies batch names", {
  ...
  expect_true(any(grepl("PXA2012-07-17, Kathleen_76 Samples_Frag_10-28-22", output))) # Adjust the batch names based on your test files
})
...
test_that("fsa_metadata returns a data frame with correct columns", {
  ...
  expected_columns <- c("file_name", "retrieved_sample_name", "batch_container_name", "fsa_version",
                        "user", "run_start_date", "run_start_time", "machine_type", "machineN_serial")
...
test_that("fsa_metadata correctly retrieves metadata", {
  ...
  expect_equal(nrow(metadata), length(list.files(file_path, pattern = "\\.fsa$")))
  ...
  expect_true(all(metadata$batch_container_name[7:12] == "Kathleen_76 Samples_Frag_10-28-22"))
})
