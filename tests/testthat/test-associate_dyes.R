test_that("associate_dyes adds dye names to data frames correctly", {
  ...
  expected_colnames <- c("Dye_ch1_6-FAM", "Dye_ch2_VIC",  "Dye_ch3_NED", "Dye_ch4_PET", "Dye_ch5_LIZ")
...
test_that("associate_dyes warns about mismatched filenames", {
  ...
  expect_message(associate_dyes(mock_fsa_batch_imp_output, file_path), "Imported filenames do not match files in the directory")
})
...
test_that("associate_dyes handles more columns than dye channels correctly", {
  ...
  mock_fsa_batch_imp_output <- list(
    file1 = data.frame(Channel1 = 1:5, Channel2 = 6:10, Channel3= 11:15,Channel4=16:20, Channel5=21:25, ExtraColumn=26:30)
  )
  #names(mock_fsa_batch_imp_output) <- c("file1.fsa")

  # Expect a message about more columns than dye channels
  expect_message(associate_dyes(mock_fsa_batch_imp_output, file_path), "1 Imported filenames do not match files in the directory.
