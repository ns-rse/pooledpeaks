# markers by set
Set_I <- c("mic_SMMS2", "mic_SMMS13", "mic_SMMS16","Shae10", "D105", "C114", "Sha104176")
Set_II<- c("mic_SMMS3", "mic_SMMS17", "mic_SMMS18", "mic_SMMS21","C102", "Shae12", "C112")
...
names(mic_set_list) <- c("Set_I", "Set_II", "Set_III", "Set_IV", "Set_V", "Set_VI", "Set_VII")
...
ch1_lists <- c("mic_SMMS2", "mic_SMMS3", "mic_13TAGA", "mic_29E6A", "mic_SM13_478", "mic_SMD28", "mic_L46951", "mic_LG1_sc276","Shae10", "C112", "Shae05")
...
ch2_lists <- c("mic_SMMS16", "mic_SMMS17", "mic_SMMS18", "mic_1F8A", "mic_15J15A","D105", "Shae12", "C146")

# channel 3; NED (yellow)
mic_SMDA23 <- c(187,191, 195, 199, 203, 207, 211, 215, 219, 223, 227, 231, 235)
mic_SMU31768 <- c(185, 188, 191, 194, 197, 200, 203, 206, 209, 212, 215, 218, 221, 224, 227)
mic_LG5_sc36b <- c(232, 235, 238, 241, 244, 247, 250, 253, 256, 259, 262, 265, 268)
...
ch3_lists <-c("mic_SMDA23", "mic_SMU31768", "mic_LG5_sc36b", "mic_sc23b", "mic_R95529","C114", "Shae14")
...
ch4_lists <- c("mic_SMMS13", "mic_SMMS21", "mic_SM13_410", "mic_LG5_sc475","Sha104176", "C102")
...
test_that("score_markers_rev3 returns expected output", {
  ...
  mock_fsa_batch_imp_output<- fsa_batch_imp(file_path, channels = 5, fourier = TRUE, saturated = TRUE,
  ...
  small_subset <- mock_fsa_batch_imp_output[1:6]  # Using a smaller subset for testing
  ...
  expect_true(all(sapply(result, function(x) all(c("pos", "hei", "wei") %in% names(x)))))
})
