...
In addition to **pooledpeaks**, library the following packages to utilize this 
package to its fullest capacity.
```{r, message=FALSE}
library(Fragman)
library(ape)
library(magrittr)
...
this process, **pooledpeaks** incorporates functionality adapted from the **Fragman** 
package, originally developed for microsatellite typing in plants. These 
...
**Additional Updates of score_markers_rev3**
...
**Note:** The author recommends setting **plotting** to TRUE and then visually inspecting the PDFs to confirm that each peak is being called as expected. If 
...
`clean_scores` row-binds all of the individual data frames and removes 
...
the individual columns .a and .b (.c, .d, etc.) with an average of the two and calculates the Jost's D between the samples.
...
processed data for population genetics. It introduces essential methods like calculating genetic distance, visualizing population structure, and creating phylogenetic trees. 

**Note:** This is a general guide intended to demonstrate the pipeline's 
capabilities, not a comprehensive or in-depth example. For detailed use cases or advanced analyses, you may need to adjust the parameters and explore additional functions in the package.
...
offline capability, and user control makes this pipeline a valuable resource for genetic studies.
